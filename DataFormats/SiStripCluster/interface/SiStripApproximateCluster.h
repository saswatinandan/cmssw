...
public:
  ...
  float avgCharge() const { 
    ...
    assert(avgCharge_ <= maxavgCharge_ && "Returning avgCharge > maxavgCharge");
     return avgCharge_; }
  bool filter() const { return ((compavgCharge_>>6)&1) ; }
  bool isSaturated() const { return ((compavgCharge_>>7)&1) ; }
  bool peakFilter() const { return ((compBarycenter_>>15) & 1); }
...
private:
  ...
  static constexpr double maxRange_ = 32767; //32767; //16383; //16383; //32767;
  static constexpr double maxBarycenter_ = 1536.;
  static constexpr double maxavgChargeRange_ = 63; //63; //255; //255; //255;
