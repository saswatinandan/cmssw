...
parser.add_argument("-v", dest="version", default='', help="name to be added in the output directory")
...
with open('makefile', 'w') as f:
    ...
    for bit in bits:
      f.write(f'{bit}bit:\n')
      f.write(f'\tbash build_env.sh {bit}bit {options.number} {options.threads} {options.remove} {options.version}\n')
    f.write(f'\nplot: {allbit}\n')
    bits = ' '.join([b for b in bits])
    f.write(f'\t python3 plot_comparison.py -b {bits} -i {options.version}')
