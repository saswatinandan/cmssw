#python3 plot_comparison_singleplot_reducedpoint.py -b 15 14 12 -w 8 -a 8 6 -o HI_wchargecut_saswati -v v3 v2
...
def draw(x_vals, y_vals, texts, ytitle, obj, rawtype):
  ...
  for idx, key in enumerate(texts.keys()):
    label = key
    if 'nochargecut' in key: label = 'v1 (RawPrime)'
    elif 'withchargecut' in key: label = 'v1 + chargecut'
    elif 'v3' in key: label = 'v3 (RawSecond)'
    plt.scatter(x_vals[key], y_vals[key], color=colors[idx], label=label)
    for i, text in enumerate(texts[key]):
      if text[0] == 15 and text[1] == 8 and key=="raw'":
            x_vals[key][i] -= 1.8
      if text[0] == 16 and text[1] == 8 and key=="HI_raw'_nochargecut":
                x_vals[key][i] -= 2.0
...
def build_array(obj, rawtype):
  ...
  texts["HI_raw'_nochargecut"] = []
  yvals["HI_raw'_nochargecut"] = []
  sizes["HI_raw'_nochargecut"] = []

  update_list('HI_wochargecut', 16, 8, rawtype, sizes, yvals, texts, "HI_raw'_nochargecut", options.events, 'Std')
  ...
  sizes["raw'"] = [(s1 -sizes["HI_raw'_nochargecut"][0])*100/sizes["HI_raw'_nochargecut"][0] for s1 in sizes["raw'"]]
  '''
  texts["HI_raw'_withchargecut"] = []
  yvals["HI_raw'_withchargecut"] = []
  sizes["HI_raw'_withchargecut"] = []
  
  update_list('HI_wchargecut', 16, 8, rawtype, sizes, yvals, texts, "HI_raw'_withchargecut", options.events, 'Std')
  sizes["HI_raw'_withchargecut"] = [(s1 -sizes["HI_raw'_nochargecut"][0])*100/sizes["HI_raw'_nochargecut"][0] for s1 in sizes["HI_raw'_withchargecut"]]
  ...
  if 'v2' in options.version:
    ...
    sizes["v2"] = [(s1 -sizes["HI_raw'_nochargecut"][0])*100/sizes["HI_raw'_nochargecut"][0] for s1 in sizes["v2"]]
  if 'v3' in options.version:
    texts['v3'] = []
    yvals['v3'] = []
    sizes['v3'] = []
    update_list('HI_wchargecut_v3', 15, 6, rawtype, sizes, yvals, texts, 'v3', options.events, 'Std')
    sizes["v3"] = [(s1 -sizes["HI_raw'_nochargecut"][0])*100/sizes["HI_raw'_nochargecut"][0] for s1 in sizes["v3"]]
  ...
  sizes["HI_raw'_nochargecut"] = [(s1 - s1)*100/sizes["HI_raw'_nochargecut"][0] for s1 in sizes["HI_raw'_nochargecut"]]
