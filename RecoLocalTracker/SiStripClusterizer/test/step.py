...
process.options = cms.untracked.PSet(
    ...
    numberOfThreads = cms.untracked.uint32(50),
...
process.rawDataRepacker = cms.EDProducer( "RawDataCollectorByLabel",
  ...
  RawCollectionList = cms.VInputTag( 'rawDataCollector' )
)
...
process.hltSiStripClusters2ApproxClusters = cms.EDProducer( "SiStripClusters2ApproxClusters",
    inputClusters = cms.InputTag( "hltSiStripRawToClustersFacility" ),
...
process.HLTDoHIStripZeroSuppressionAndRawPrimeRepacker = cms.Sequence( process.hltSiStripRawToClustersFacility + process.hltSiStripClusters2ApproxClusters + process.rawDataRepacker +  process.rawPrimeDataRepacker )

process.HLT_EphemeralPhysics_v10 = cms.Path(process.HLTBeginSequence+ process.hltL1sL1APhysicsMaskingL1ZeroBiasCopy+process.hltPreEphemeralPhysics+process.HLTDoLocalPixelSequence + process.HLTRecoPixelTracksSequence + process.HLTDoHIStripZeroSuppressionAndRawPrimeRepacker + process.hltMeasurementTrackerEvent + process.hltPixelVerticesSoA + process.hltPixelVertices + process.hltTrimmedPixelVertices + process.HLTIterativeTrackingIter02 + process.HLTEndSequence)
process.schedule = cms.Schedule(*[ process.L1RePack_step, process.HLTriggerFirstPath, process.Status_OnCPU, process.Status_OnGPU,process.HLT_EphemeralPhysics_v10,process.Dataset_EphemeralHLTPhysics0,process.FEVTDEBUGHLToutput_step])
