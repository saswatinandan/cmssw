...
# -------------------------
def dataset_from_generator(files, feature_to_use, batch_size, class_weight_dict=None, shuffle_buffer=0):
    """
    ...
    ds = tf.data.Dataset.from_generator(
        lambda: training_generator(files, feature_to_use, class_weight_dict, batch_size),
...
train_dataset = dataset_from_generator(
    train_files, feature_to_use, batch_size=batch_size_train, class_weight_dict=class_weight_dict, shuffle_buffer=200
).prefetch(tf.data.AUTOTUNE)

val_dataset = dataset_from_generator(
    val_files, feature_to_use, batch_size=8000, class_weight_dict=class_weight_dict, shuffle_buffer=200
).prefetch(tf.data.AUTOTUNE)

test_dataset = dataset_from_generator(
    test_files, feature_to_use, batch_size=8000
).prefetch(tf.data.AUTOTUNE)
