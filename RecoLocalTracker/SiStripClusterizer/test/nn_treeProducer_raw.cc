  ...
  for (const auto& detSiStripClusters : *clusterCollection) {
	...
	noises.push_back(theNoise_->getNoise(strip, detNoiseRange));
	 ...
	 if(i<10) {
      ...
      auto gp = GlobalPoint(mean_x, mean_y, mean_z);
      eta     = gp.eta();
      phi     = gp.phi();
	...
	for (auto const& hit : trackRef->recHits()) {
	     ...
	     /*else {
                tsos = thePropagator->propagate(tkTT.outermostMeasurementState(), geomDet->surface());
		if ( tsos.isValid() ) {
	           recotrk_tsosCache[trackRef] = tsos;
                   //std::cout << "propagation failed for innermost but sucessful for outermost for recotrk" << std::endl;
		   break;
		}
	     }*/
         }
     ...
     if ( firstcluster ) {
      ...
      dr_reco_vs_pixel = 99;
      if ( recotrk ) {
	      //std::cout << "reco,pt: "<< recotrk->pt() << "\teta: " << recotrk->eta() << "\tphi: " << recotrk->phi() << std::endl; 
        for (const auto& [trackRef, tsos] : pixeltrk_tsosCache) {
	     float dr = deltaR(recotrk->eta(), trackRef->eta(), recotrk->phi(), trackRef->phi()); 
	     //float dp = abs(recotrk->pt() - trackRef->pt()) / trackRef->ptError();
	     //std::cout << "pixel,pt: "<< trackRef->pt() << "\teta: " << trackRef->eta() << "\tphi: " << trackRef->phi() << "\tdr: " << dr << std::endl;
             if (!(dr < 0.05 && dr < dr_reco_vs_pixel)) continue;
	     auto localValues = stripCPE->localParameters(stripCluster, *stripDet, tsos);
	     LocalPoint clusterLocal = localValues.first;
	     LocalPoint trackLocal = geomDet->surface().toLocal(tsos.globalPosition());
	     pixeltrk_dr_min = abs(trackLocal.x() - clusterLocal.x());
             pixeltrk = &(*trackRef);
	     dr_reco_vs_pixel = dr;
	     //dp_reco_vs_pixel = abs(recotrk->pt() - pixeltrk->pt()) / pixeltrk->ptError();
     ...
     /* if (recotrk && pixeltrk) {
	      std::cout << "target: " << target << "\tx:" << mean_x << "y:" << mean_y << "z:" << mean_z << "\tsize:" << size << "\tcharge:" << charge << "\tdr_reco_vs_pixel: " << dr_reco_vs_pixel << std::endl;
	  ...
	  for (auto const& hit : trackRef->recHits()) {
		...
		/*else {
		   ...
		   if ( tsos.isValid() ) {
		     hlttrk_tsosCache[trackRef] = tsos;
                     //std::cout << "propagation failed for inner for hlt trk" << std::endl;
		     break;
		   }
		}*/
             }
