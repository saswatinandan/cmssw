...
void nn_tupleProducer_raw::analyze(const edm::Event& event, const edm::EventSetup& es) {
  ...
  //std::cout << "trkcluster: " <<  trkcluster << std::endl; 
  ...
  for (const auto& detSiStripClusters : *clusterCollection) {
	...
	if (adc    [strip - firstStrip] >= 254) n_saturated += 1;
      ...
      //std::vector<int> pixeltrks = {8,9,10,26};
      dr_min_pixelTrk = 99.;
      ...
      for ( const auto & trk : hlttracks) {
	   //   std::cout << "trk algo " << trk.algo() << std::endl;
		...
		//vrtx_z = trk.dz(vertices.at(0).position());
                ...
                // Get cluster local position using CPE
		//found = true;
	...
	//		            pow( (trackLocal.y() - clusterLocal.y()), 2 ) +
	//			    pow( (trackLocal.z() - clusterLocal.z()), 2 )); // u-residual
                if (dr < dr_min_pixelTrk) {
                  dr_min_pixelTrk = dr;
	//	std::cout << trackLocal.x() << "\t" << clusterLocal.x() << "\t" << geomDet->surface().toGlobal(trackLocal).x() << "\t" << tsos.globalPosition().x() << "\t" << trackLocal.y() << "\t" << clusterLocal.y() << "\t" << geomDet->surface().toGlobal(trackLocal).y() << "\t" << tsos.globalPosition().y() << "\t" << trackLocal.z() << "\t" << clusterLocal.z() << "\t" <<  geomDet->surface().toGlobal(trackLocal).z() << "\t" << tsos.globalPosition().z() << "\t" << dr_min_pixelTrk << "\t" << barycenter << std::endl;
		  trk_pt = trk.pt();
                }
	      }
	      //std::cout << "validhit " << validhit << std::endl;
      }
      //if (abs(dr_min_pixelTrk) == 99 && signal) 
	     // std::cout << "dr_min_pixelTrk " << signal << dr_min_pixelTrk << std::endl; 
      //if(signal &&  (dr_min_pixelTrk==99))std::cout << "found " << found << "\t" << found_strip << "\t" << found_hit << "\t" << found_sos << std::endl;
      if (signal) {
	      //std::cout << "barycenter " << barycenter << "\t" << size << "\t" << charge << "\t" << detId << std::endl;
	      signalTree->Fill();
