...
SiStripClusters2ApproxClusters::SiStripClusters2ApproxClusters(const edm::ParameterSet& conf) {
  ...
  input_training_vars = conf.getParameter<std::vector<std::string>>("dnnvars");
  cutvalue = conf.getParameter<double>("dnncutvalue");
  ...
  for (const auto& detClusters : clusterCollection) {
    ...
    for (const auto& cluster : detClusters) {
	 ...
	 for (auto const& hit : trk.recHits()) {
      ...
      std::vector<std::vector<float>> dnn_input_values = DNNStripCluster(cluster, tkGeom, detId, p, input_training_vars);
...
void SiStripClusters2ApproxClusters::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<std::vector<std::string>>("dnnvars", {});
  desc.add<double>("dnncutvalue",0);
